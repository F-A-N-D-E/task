...
router.get('/create', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (
      ...
      !validStringLength(query.title) ||
      !validStringLength(query.message_appeal)
    ) {
      res.send({
        err: 'Вы превысили максимальную длину текста в 255 символов',
      });
      return;
    }
...
router.get('/betweenDate', async (req: Request, res: Response) => {
  // ищет по диапазону
  try {
    ...
    if (!validDateFrom && !validDateTo) {
      // некорректная дата
      res.send({
        err: 'Пожалуйста, введите дату в формате - "YYYY-MM-DD" или воспользуйтесь встроенным календарем',
      });
      return;
    }
    ...
    } else if (validDateFrom && !validDateTo) {
      // есть только от куда
      [data] = await sequelize.query(
...
router.get('/work/:id', async (req: Request, res: Response) => {
  // ставит статус work
  try {
    ...
    await sequelize.query(
      `UPDATE appeal SET status = 'work', respon = '' WHERE id = :id`,
      {
        replacements: { id: id },
      }
    );
...
router.get('/cancelWork', async (req: Request, res: Response) => {
  // ставит всем work статус cancel
  try {
    await sequelize.query(`
